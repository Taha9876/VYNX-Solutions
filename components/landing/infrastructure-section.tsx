...
const regions = [
  { name: "Websites", nodes: 58, status: "delivered" },
  { name: "Animated Sites", nodes: 34, status: "delivered" },
  { name: "Custom Software", nodes: 21, status: "delivered" },
  { name: "Automations", nodes: 47, status: "deployed" },
];
...
export function InfrastructureSection() {
  ...
  return (
    <section id="infra" ref={sectionRef} className="relative py-16 md:py-32 lg:py-40 overflow-hidden">
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Every project we ship is engineered to drive real business outcomes — more leads, more revenue, less manual work.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-6xl md:text-8xl lg:text-[10rem] font-display leading-none" data-count="120" data-count-suffix="+">120+</span>
                <span className="text-2xl text-muted-foreground">projects</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Shipped across 8 industries — from startups to established brands seeking a digital edge.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display" data-count="98" data-count-suffix="%">98%</span>
              <span className="block text-sm text-muted-foreground mt-2">Client retention rate</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">&lt;30d</span>
              <span className="block text-sm text-muted-foreground mt-2">Average delivery time</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} projects</span>
            </div>
          ))}
        </div>
      </div>
